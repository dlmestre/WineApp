...
def get_updated_databases(path):
    ...
    for a_file in original_files:
        ...
        handledict_obj = handleDictionary(a_file,True,location=path)
        working_wines.extend(handledict_obj.found_wines)
        not_working_wines.extend(handledict_obj.not_found_wines)
    try:
        found_wines = pd.DataFrame()
        found_wines["wine"] = [tup[0] for tup in working_wines]
        found_wines["dictionary wine"] = [tup[1] for tup in working_wines]
        found_wines.to_csv(os.path.join(path,"found_wines.csv"),encoding="utf-8")
    except Exception as e:
        log(e)
    try:
        not_found_wines = pd.DataFrame()
        not_found_wines["wine"] = not_working_wines
        not_found_wines.to_csv(os.path.join(path,"not_found_wines.csv"),encoding="utf-8")
    except Exception as e:
        log(e)
    return files
...
log("It took {0} seconds".format(time.time() - init_time))
